...
APPROVAL_STORE_PATH = Path(WORKSPACE_DIR) / "sandbox" / "command_approvals.json"
# 兼容旧代码/测试的进程内缓存；真实审批状态以 sandbox/command_approvals.json 为准，
# 这样 registry.execute_tool_isolated() 的不同子进程也能共享审批 token。
...
def _approval_store_path() -> Path:
    """Return the shared approval store path, creating sandbox when needed."""
    path = APPROVAL_STORE_PATH
    path.parent.mkdir(parents=True, exist_ok=True)
    return path


def _read_approval_store(now: float = None, prune_expired: bool = True) -> dict:
    """Read shared pending command approvals from disk.

    The store intentionally lives under sandbox so isolated tool child processes
    can validate tokens generated by previous child processes.  Malformed files
    are treated as empty to avoid blocking safe command execution forever.
    """
    ...
    if not isinstance(data, dict):
        return {}
    approvals = data.get("approvals", data)
    ...
    for token, record in approvals.items():
        if not isinstance(token, str) or not isinstance(record, dict):
            changed = True
            continue
        if prune_expired and now > record.get("expires_at", 0):
            changed = True
            continue
        cleaned[token] = record
    if changed and prune_expired:
        _write_approval_store(cleaned)
    return cleaned


def _write_approval_store(approvals: dict) -> None:
    """Atomically write shared pending command approvals to disk."""
    path = _approval_store_path()
    payload = {
        "version": 1,
...
def _store_approval_record(token: str, record: dict) -> None:
    approvals = _read_approval_store()
    approvals[token] = record
    _write_approval_store(approvals)
    _PENDING_COMMAND_APPROVALS[token] = record


def _pop_approval_record(token: str) -> None:
    if not token:
        return
    approvals = _read_approval_store()
    if token in approvals:
        ...
        _write_approval_store(approvals)
    _PENDING_COMMAND_APPROVALS.pop(token, None)
...
def _is_valid_approval(command: str, cwd: str, assessment: dict, approval_token: str) -> bool:
    ...
    if time.time() > record.get("expires_at", 0):
        _pop_approval_record(approval_token)
        return False
