...
def test_save_prunes_dangling_dependency(tmp_path):
    # A validation task depending on a task_id that is not in the DAG would be
    # permanently unready; persistence must strip the dangling dependency so the
    # checkpoint can run and officially score improved code.
    state = {
        "tasks": [
            {"task_id": "impl", "goal": "write solution", "type": "implementation", "status": "verified"},
            {"task_id": "val", "goal": "run eval", "type": "validation", "depends_on": ["missing_id"]},
        ]
    }
    save_todo_state(tmp_path, state)
    loaded = load_todo_state(tmp_path)
    val = next(t for t in loaded["tasks"] if t["task_id"] == "val")
    assert val["depends_on"] == []
    # With the dangling dep pruned, the validation task is now ready to run.
    assert any(t["task_id"] == "val" for t in ready_tasks(loaded, phase="run"))


def test_save_keeps_valid_dependency(tmp_path):
    state = {
        "tasks": [
            {"task_id": "impl", "goal": "write solution", "type": "implementation", "status": "pending"},
            {"task_id": "val", "goal": "run eval", "type": "validation", "depends_on": ["impl"]},
        ]
    }
    save_todo_state(tmp_path, state)
    loaded = load_todo_state(tmp_path)
    val = next(t for t in loaded["tasks"] if t["task_id"] == "val")
    assert val["depends_on"] == ["impl"]
    # impl is not yet verified, so the validation checkpoint is not ready.
    assert not any(t["task_id"] == "val" for t in ready_tasks(loaded, phase="run"))


def test_load_prunes_dangling_dependency_written_directly(tmp_path):
    # Simulate a raw planner dump written straight to disk with a bad dep id.
    todo_state_path(tmp_path).parent.mkdir(parents=True, exist_ok=True)
    todo_state_path(tmp_path).write_text(json.dumps({
        "version": 1,
        "tasks": [
            {"task_id": "val", "goal": "run eval", "type": "validation", "depends_on": ["ghost"]},
        ],
    }), encoding="utf-8")
    loaded = load_todo_state(tmp_path)
    assert loaded["tasks"][0]["depends_on"] == []
