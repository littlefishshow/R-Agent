...
### 0. 长论文任务拆解与上下文拆解规范
当论文很长、结构复杂或用户要求精读时，必须先拆解任务和上下文，再进入逐段阅读；不要把整篇长论文一次性塞进主上下文后直接生成笔记。

触发条件包括但不限于：
- PDF 超过约 30 页，或主文虽短但 Appendix / Supplementary Material 很长。
- 图表、公式、算法框、prompt、实验设置、附录细节很多。
- 用户明确要求精读、复现、公式推导、实验公平性、代码对应关系或 100 页级长文细读。
- 抽取文本、OCR 文本、截图索引或候选证据接近单轮上下文预算。

执行策略：
1. **先建索引再阅读**：先定位 PDF、抽取全文、建立章节/页码/图表/公式/附录索引，并写入 `sandbox/read_paper/<paper_stem>/`；主上下文只保留索引摘要、当前阶段目标、关键证据锚点和待解决问题。
2. **按章节/页码/关注点拆任务**：根据论文结构拆成可独立阅读的块，例如 Introduction/Related Work、Method/Theory、Experiments/Ablation、Appendix/Limitations、Figures/Tables/Algorithms、Reproducibility/Cost/Fairness。长论文可进一步按页码范围或附录小节拆分。
3. **每个子任务只读局部材料**：子任务不得回灌完整原文、完整网页、完整截图 OCR 或完整子对话；只输出结构化局部笔记，包括已读范围、核心论点、方法/公式/实验细节、关键图表与数值、页码/章节/图表号证据、材料缺口、冲突点和下一步建议。
4. **阶段性交接要可合并**：每个阶段应在 `sandbox/read_paper/<paper_stem>/` 生成 `stage_summary.md`、`chunk_notes_*.md` 或等价摘要，记录：已读范围、关键结论、证据锚点、未解决问题、需要父任务统一口径的地方。
5. **父任务统一合成**：最终阶段负责把局部笔记按论文主线重新组织成最终 Markdown，统一术语、消除重复、检查局部结论冲突、补齐遗漏，并把关键证据合并到第 1 章、第 3 章、实验/附录/批判性总结等对应位置。
6. **上下文预算优先**：当抽取文本或中间材料过长时，先写入 sandbox/artifact 并建立索引；当前对话只保留摘要、证据定位和下一步任务。禁止把整篇长论文全文、大段附录、整批 OCR 文本或所有子任务完整上下文复制到主对话。
7. **拆解不能降低质量门槛**：即使采用多阶段或多子任务阅读，也必须满足第 1 章小白解释、第 3 章主线串读、Appendix/Limitations 覆盖、图表/公式就地解释、强制重检查和最终验收要求。最终 Markdown 应保留“重检查记录”，必要时说明分块阅读范围与补漏情况。
