...
### 3. 全局扫描：先明确论文大体工作
重点阅读 Abstract、Introduction、Conclusion，并提取：
- 行业内/研究领域的大体现状是什么？
- 论文要解决的问题是什么？
- 现有方法哪里不够？包括性能、泛化、效率、成本、数据依赖、可解释性、鲁棒性等。
- 作者提出了什么方法、框架、数据集、理论或训练策略？
- 作者声称的主要贡献是什么？逐条列出。
- 结果提升了多少？说明提升对应的数据集、指标、baseline 和实验设定。
- 一句话概括：这篇论文到底做了什么，以及为什么重要。

### 4. 论文主线串读：按行文顺序融合问题、方法、图表和公式
本节是阅读笔记的主体，目标是让读者像跟着论文作者的论证路线读原文，而不是读一组彼此割裂的清单。**必须按论文实际行文顺序推进**：作者先提出什么问题，为什么旧方法不够；随后如何形式化问题、给出方法；每个公式在解决哪个定义/优化问题；每张图表在支撑哪一步论证；实验结果如何回扣前面的假设。

强制写作原则：
1. **先写叙事地图，再写节点**：在第 4 节开头用 3-7 句话或一个短列表交代全文路线，例如 `Introduction → Problem Definition → Method → Training Objective → Experiments → Ablation/Analysis → Limitations`。后续小节必须沿这条路线展开。
2. **以连续段落为主，列表为辅**：每个节点先用自然段说明“上一节留下什么问题 → 本节如何推进 → 图表/公式提供什么证据 → 下一节为什么需要继续”，再用少量 bullet 补充符号、数值或边界。不要只写“原文位置/图表说明/我的判断”的表单式清单。
3. **图表和公式必须就地服务论证**：如果一张图是方法概览，就放在方法第一次被解释的位置；如果一张表是主结果，就放在实验主结果段落；如果公式定义训练目标，就放在作者从问题定义过渡到优化方法的段落。禁止把“图表索引”“公式索引”“方法清单”分别堆在不同位置后再让读者自行拼接。
4. **每个叙事节点都要回答连接问题**：
   - 这一节点承接了前文哪个问题？
   - 作者在这里引入了什么概念/假设/机制？
   - 图表、公式或实验结果如何支撑这个机制？
   - 这个证据是否充分，下一步自然引出什么？
5. **保留可扫读锚点，但不要牺牲可读性**：小节标题可包含 `Sec.x / p.x / Figure x / Eq.x`，但正文必须是顺畅解释；公式后要立即解释符号和通俗含义；图表后要说明它在当前论证中的作用。
6. **避免重复割裂**：第 5-8 节可以做总结、索引和批判，但不得替代第 4 节主线串读。第 4 节已经讲过的图表/公式，后文只回指和评价，不要重新孤立复述。

推荐节点结构（不是机械模板，写作时应合并成通顺段落）：
1. **过渡句**：承接上一个节点，说明为什么作者需要进入当前部分。
2. **原文位置锚点**：章节号、页码、图号/表号/公式号。
3. **作者论证**：这一段在整篇论文中承担什么作用。
4. **就地插入图表/公式**：截图和公式紧跟解释出现。
5. **解释与判断**：解释符号、图表趋势、实验设置，并说明证据强弱、反例和边界。
6. **下一步引出**：用一句话说明作者接下来为什么要做下一节/下一组实验。
...
### 4.2 公式与数学定义的就地解释规则
公式不要集中堆在“公式表”里，而要放在论文主线中首次需要它的位置：
...
- 理论分析公式：放在 Appendix/理论解释处，明确它是严格保证、直觉解释还是附录假设。
...
### 10.5 Markdown 数学公式与 KaTeX 安全规则
...
## 4. 论文主线串读：按行文顺序融合问题、方法、图表和公式
> 本节是阅读笔记的主体。先给出“叙事地图”，再按论文行文顺序用连续段落串联问题、方法、公式、图表和实验。不要把主线、图表、公式拆成三个孤立清单；每张图/表/公式都必须嵌入它支撑的论证附近。

### 4.0 叙事地图：作者如何一步步推进全文
用 3-7 句话说明论文路线：从背景问题出发，如何进入任务定义/方法，如何设计训练或实验，最后如何用结果、消融和局限回扣核心假设。

### 4.x 原文节点标题（Sec.x / p.x / Figure x / Table x / Eq.x）
先用一段自然语言承接上一节点：作者为什么要进入这里？这一节点在全文论证链中解决什么问题？
...
接着用连续段落解释：公式里的符号分别是什么，图表展示了什么趋势或结构，它如何支撑作者当前论点；如果证据不足或有反例，也在这里直接写出。最后用一句话自然引出下一节点。

## 5. 论文主线总结
### 5.1 问题定义：任务、输入、输出、成功标准
### 5.2 核心假设与证据链
### 5.3 方法流程总览
### 5.3.1 监督信号/标签/critique/proxy 来源表（如适用）
| 信号 | 用途 | 取值/公式 | 由谁产生 | 是否依赖 ground truth/oracle/evaluator | 训练/评测可用阶段 | 附录证据 | 迁移成本 |
|---|---|---|---|---|---|---|---|

### 5.4 最小可运行版本 / sanity check

## 6. 关键公式索引（可选）
> 这里只做索引，详细解释必须已经在第 4 节主线串读中出现。
| 公式号 | 对应主线节点 | 作用 | 详解位置 |
|---|---|---|---|
...
### 8.1 Appendix 关键细节
### 8.2 作者承认的局限
### 8.3 我记录的不利证据/反例/边界条件
### 8.4 与旧论文/经典思想/相邻领域的关系
...
- 是否覆盖关键图表、案例、原始输出，并解释作者分析？
- 是否已对关键 Figure/Table 生成并插入截图，且 Markdown 图片路径可用？
- 是否讲清任务输入、输出、成功标准和难点？
- 是否找到了核心假设，而不是只复述方法？
- 是否按论文行文顺序完成主线串读，把问题、方法、图表、公式、实验连接成一个统一叙事？
- 第 4 节是否先给出叙事地图，并用连续段落说明“上一节点的问题 → 当前节点的机制/证据 → 下一节点的引出”，而不是表单式罗列？
- 图表和公式是否都出现在它们实际支撑的论证附近，并解释它们如何服务当前主线？
- 是否给出方法整体流程图或清晰架构描述？
- Method 章节是否已经讲清所有关键步骤、公式、变量、超参数、训练/推理差异和模块连接；是否存在只用高层直觉概括、但遗漏论文具体机制的地方？
- 是否在相关主线节点就地解释关键公式、符号和通俗含义，而不是集中堆公式？
...
- 是否阅读 Appendix 与 Limitations，并记录不利证据、边界条件和作者未解释的问题？
