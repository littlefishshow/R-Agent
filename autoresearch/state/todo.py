...
def prune_dangling_dependencies(state: dict) -> dict:
    """Drop depends_on ids that reference no task in the *complete* DAG.

    A single mistyped/missing dependency id would otherwise make a task
    permanently un-ready (dependencies_satisfied() treats a missing id as
    "not done" forever), silently stalling the whole loop — the validation
    checkpoint never runs, so improved code is never officially scored. DAG
    reference integrity is a mechanical invariant the framework should own
    rather than trusting the planner to always emit a consistent DAG.

    This runs only on a fully-assembled DAG (persisted state), never on the
    intermediate fragments that merge_todo_state assembles, so a plan that
    references an existing-but-not-yet-merged task is not stripped prematurely.
    """
    ...
    for task in tasks:
        ...
        kept = [dep for dep in deps if dep in valid_ids and dep != task["task_id"]]
...
def load_todo_state(root: str | Path) -> dict:
    ...
    try:
        ...
        return empty_todo_state()
    # Enforce DAG reference integrity on read too, so a todo_state written by any
    # other path (e.g. a raw planner dump) cannot leave a validation checkpoint
    # permanently unready due to a dangling depends_on id.
    return prune_dangling_dependencies(data if isinstance(data, dict) else None)


def save_todo_state(root: str | Path, state: dict) -> Path:
    path = todo_state_path(root)
    # Persisted state is a complete DAG: enforce reference integrity here so a
    # planner-produced depends_on pointing at a non-existent task cannot trap a
    # validation checkpoint in a permanently-unready state.
    normalized = prune_dangling_dependencies(state)
