...
def _v2_settings_kwargs(
    ...
    llm_request_timeout=300.0, plan_max_personas=2,
...
def _v2_properties():
    return {
        ...
        "plateau_patience": {"type": "integer", "description": "连续多少轮 Pareto 无改进后触发重规划(收敛信号 K)。默认不会因为 plateau 暂停；autoresearch 会持续寻找更好解，除非预算耗尽、program.md 中的项目级完成标准满足，或用户停止。", "default": 3},
        "trace_rounds": {"type": "boolean", "description": "是否把每轮完整上下文(parent_context+system/user prompt+LLM 原始返回+选中动作+观察)dump 到 .autoresearch/round_traces/round_NNN_*.json 供事后排查；默认开启。", "default": True},
        "debug_mode": {"type": "boolean", "description": "是否开启 debug 模式：写 .autoresearch/debug_logs/debug.jsonl 和 inflight.json，显示当前卡在 LLM/shell/phase 的哪一步。可删除 .autoresearch/DEBUG 或用 /autoresearch debug off 关闭。", "default": False},
        "llm_request_timeout": {"type": "number", "description": "单次 LLM 请求超时秒数。默认 300s，让 Execute 子进程有足够时间读写文件；超时会记录到 debug_logs/inflight，并由任务 attempt 计数决定是否重试或 replan。", "default": 300.0},
