...
def make_evaluate_handler():
    ...
    def handler(ctx: PhaseContext) -> PhaseResult:
        ...
        if isinstance(best, dict):
            primary_name = str(best.get("primary_metric_name") or "")
            metrics = best.get("metrics") if isinstance(best.get("metrics"), dict) else {}
            if not primary_name and metrics:
                primary_name = str(next(iter(metrics)))
            if primary_name:
                primary_value = metrics.get(primary_name)
            if primary_value is None:
                primary_value = metrics.get("primary_metric")
            directions = best.get("metric_directions") if isinstance(best.get("metric_directions"), dict) else {}
            higher = directions.get(primary_name)
            if higher is None:
                higher = directions.get("primary_metric")
            try:
                solved = is_metric_solved(float(primary_value), criteria)
            except Exception:
                solved = False
        ...
        project = ctx.project_text
        conclusion_parts = [summary, f"pareto_candidates={len(pareto)}"]
        if primary_name and primary_value is not None:
            conclusion_parts.append(f"metric {primary_name}={primary_value}")
        if criteria is not None:
            conclusion_parts.append(
                f"completion {criteria.metric_name} {criteria.operator} {criteria.threshold}: "
                f"{'met' if solved else 'not met'}"
            )
        if solved or ctx.signals.solved:
            conclusion_parts.append("next=pause (solved target reached)")
        elif major:
            conclusion_parts.append("next=replan required")
        else:
            conclusion_parts.append("next=continue")
        conclusion_line = "- " + "; ".join(conclusion_parts)
        if "## 短期结论" in project:
