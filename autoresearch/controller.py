...
class ThreeStepController:
    ...
    def __init__(self, settings, *, loop=None, monitor=None, run_id: str = ""):
        ...
        self.run_id = run_id
        self._step_index = 0
        # Plateau / stall accounting so the loop cannot spin forever when it is
        # making no metric progress and there is no budget cap to brake it.
        self._forced_replans = 0
        self._last_best_experiment_id = None
    ...
    def _next_after_conclude(self, signals: PhaseSignals) -> tuple[str, str]:
        ...
        # Track whether best actually advanced this conclude. This is the real
        # "did we make progress" signal, independent of any budget cap.
        best_id = gate.get("best_experiment_id")
        made_progress = bool(gate.get("pareto_changed")) or (
            best_id is not None and best_id != self._last_best_experiment_id
        )
        if made_progress:
            self._replans_since_progress = 0
        self._last_best_experiment_id = best_id

        patience = max(1, int(getattr(signals, "plateau_patience", 3) or 3))
        plateau_counter = int(gate.get("plateau_counter") or 0)
        major_error = bool(gate.get("blocked_reason") == "major_error") or bool(getattr(signals, "major_error", False))

        if has_blocking_failed_tasks(todo_state):
            return "plan", "failed task requires replanning"
        if major_error:
            return "plan", "major error during attempt: replanning"

        # Plateau brake: once we have plateaued for `patience` scored experiments
        # (no pareto/best change), the current DAG is not moving the metric. Force
        # a replan to try a different direction — but bound how many times we
        # replan without any progress so a genuinely stuck project pauses instead
        # of spinning to max_steps (critical when there is no budget cap).
        if plateau_counter >= patience:
            if self._replans_since_progress >= patience:
                return "pause", (
                    f"no metric progress after {self._replans_since_progress} replans "
                    f"(plateau_counter={plateau_counter}); pausing for user"
                )
            self._replans_since_progress += 1
            self._forced_replans += 1
            return "plan", (
                f"plateau detected (counter={plateau_counter}>=patience={patience}); "
                f"replanning a new direction [{self._replans_since_progress}/{patience}]"
            )
    ...
    def _run_attempt(self, signals: PhaseSignals) -> tuple[PhaseResult, str, str]:
        ...
        if combined_signals.get("solved"):
            # A solved run can end directly from attempt. Still run the cheap
            # deterministic evaluate handler so project.md/gate_signals.json
            # reflect the final best metric; otherwise humans see stale status.
            solved_ctx = PhaseContext(
                phase="conclude",
                root=self.root,
                program_text=self.read_program(),
                project_text=project_text,
                signals=self.build_signals("conclude", {"solved": True}),
                loop=self.loop,
            )
            eval_result = self._evaluate_handler(solved_ctx) or PhaseResult()
            project_text = self._persist_result(solved_ctx, eval_result)
            result = PhaseResult(
                project_text=project_text,
                signals_update=combined_signals,
                summary="; ".join(s for s in (combined_summary, eval_result.summary) if s) or "attempt: solved",
            )
    ...
    def _run_conclude(self, signals: PhaseSignals) -> tuple[PhaseResult, str, str]:
        ...
        return PhaseResult(project_text=project_text, summary=summary,
                           signals_update={"transition_reason": reason}), project_text, next_phase
    ...
    def step(self, extra_signals: Optional[dict] = None) -> dict:
        ...
        try:
            if phase == "plan":
                ...
                # Prefer the accurate transition reason computed by
                # _next_after_conclude (solved / plateau replan / plateau pause /
                # budget) instead of a single hardcoded label.
                reason = (result.signals_update or {}).get("transition_reason") or "conclusion recorded"
            else:
...
def _step_system_prompt(policy, context: dict) -> str:
    ...
    return (
        ...
        "If the step context, state.json, experiment_memory, or tool outputs become too large, use archive_subtask to compress the most important context and record a summary plus next steps before continuing.\n"
        "For difficult planning problems, first inspect local project context; if external background could materially help, use web_search to find sources and web_extract to read the most relevant pages before deciding.\n"
